...
export function Toolbar({
  ...
  return (
    <div className="toolbar">
      ...
      {!running && <>
        <button className="btn btn-step" onClick={onStep} disabled={appState === 'error'}>↓ Step    <kbd>F7</kbd></button>
        <button className="btn btn-step-over" onClick={onStepOver} disabled={appState === 'error'}>↷ Over    <kbd>F8</kbd></button>
        <button className="btn btn-step-out" onClick={onStepOut} disabled={appState === 'error'}>↵ Out     <kbd>F10</kbd></button>
        <button className="btn btn-back" onClick={onStepBack} disabled={appState === 'error' || histLen === 0} title={`Undo last step (${histLen} available)`}>⟲ Back{histLen > 0 ? ` (${histLen})` : ''}</button>
      </>}
      ...
      <button className="btn btn-reset" onClick={onReset}>↺ Reset  <kbd>F6</kbd></button>
    </div>
  );
}